...
Vendor - Submit penawaran ke project (form + save bid ke database) | LPrecast | 2. Tender | Not started | April 30, 2026 | Menunggu pondasi validasi sistem selesai agar form submit bisa langsung pakai guard final.
Sistem - Validasi submit bid vendor (relasi ke tender, 1 vendor 1 bid) | LPrecast | 2. Tender | Done | April 30, 2026 | Tambah guard repository + vendor server action untuk cek tender exists/open dan duplicate submission. Tambah DB unique constraint `(tender_id, vendor_id)` untuk cegah race condition. Validasi: `pnpm typecheck`
Sistem - Validasi deadline tender (tidak bisa apply setelah close) | LPrecast | 2. Tender | Done | April 30, 2026 | Tambah field `submission_deadline_at` di tender, form publish admin, dan guard repository untuk blok submit jika status non-open atau deadline sudah lewat. Validasi: `pnpm typecheck`
