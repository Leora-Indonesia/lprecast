...
export function VendorApprovalChecklistPreview({
  checkedItems,
  onCheck,
}: VendorApprovalChecklistProps) {
  return (
      ...
      <Tabs defaultValue="admin">
        ...
        <TabsContent value="admin" className="mt-6 w-full max-w-full space-y-4">
          {adminChecklist.map((section) => (
            <section
              key={section.id}
              className="box-border w-full max-w-full min-w-0 space-y-3 overflow-hidden rounded-lg border bg-background p-4"
            >
              <div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {section.items.map((item) => (
                  <div
                    key={item.id}
                    className="flex min-w-0 items-start space-x-2"
                  >
                    <Checkbox
                      id={item.id}
                    ...
                    <Label
                      htmlFor={item.id}
                      className="min-w-0 cursor-pointer text-sm leading-snug font-normal break-words"
                    >
                      {item.label}
                      {item.isCritical && (
                        <Badge
        ...
        <TabsContent
          value="survey"
          className="mt-6 w-full max-w-full space-y-4"
        >
