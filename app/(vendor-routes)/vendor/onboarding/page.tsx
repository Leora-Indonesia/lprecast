...
function transformToDraftData(formData: VendorRegistrationFormData) {
  return {
    currentStep: 1,
    company_info: {
...
export default function VendorOnboardingPage() {
    ...
    async function checkStatus() {
      try {
        ...
        if (status.status === "submitted" || status.status === "under_review") {
          toast.info("Pendaftaran sudah diajukan", {
            description: "Anda tidak dapat mengedit data yang sudah diajukan.",
          })
        ...
        if (status.status === "approved") {
          toast.info("Pendaftaran sudah disetujui", {
            description: "Anda sudah menjadi vendor aktif.",
          })
          router.push("/vendor/dashboard")
          return
        }
        ...
        if (status.hasDraft) {
          ...
          if (draftResult.success && draftResult.data) {
            ...
            const transformedValues: VendorRegistrationFormData = {
              company_info: {
                nama_perusahaan: draft.company_info.nama_perusahaan,
                email: draft.company_info.email,
                nama_pic: draft.company_info.nama_pic,
                kontak_pic: draft.company_info.kontak_pic,
                website: draft.company_info.website,
                instagram: draft.company_info.instagram,
                facebook: draft.company_info.facebook,
                linkedin: draft.company_info.linkedin,
                contact_1: draft.company_info.contacts.find(
                  (c) => c.sequence === 1
                ) || { no_hp: "", nama: "", jabatan: "" },
                contact_2: draft.company_info.contacts.find(
                  (c) => c.sequence === 2
                ) || { no_hp: "", nama: "", jabatan: "" },
                contact_3: draft.company_info.contacts.find(
                  (c) => c.sequence === 3
                ) || { no_hp: "", nama: "", jabatan: "" },
              },
              legal_documents: {
                ktp_file: undefined,
                npwp_nomor: draft.documents.npwp_number || "",
                npwp_file: undefined,
                nib_nomor: draft.documents.nib_number || "",
                nib_file: undefined,
                siup_file: undefined,
                compro_file: undefined,
              },
              operational: {
                bank: {
                  bank_nama: draft.operational.bank_account.bank_name,
                  bank_nomor: draft.operational.bank_account.account_number,
                  bank_atas_nama:
                    draft.operational.bank_account.account_holder_name,
                },
                factory_address: {
                  alamat_detail: draft.operational.factory_address.address,
                  provinsi_id: "",
                  provinsi_name: draft.operational.factory_address.province,
                  kabupaten_id: "",
                  kabupaten_name: draft.operational.factory_address.kabupaten,
                  kecamatan: draft.operational.factory_address.kecamatan,
                  kode_pos: draft.operational.factory_address.postal_code,
                },
                products: draft.operational.products.map((p) => ({
                  name: p.name,
                  satuan: p.satuan,
                  price: p.price,
                  dimensions: p.dimensions,
                ...
                delivery_areas: draft.operational.delivery_areas.map((a) => ({
                ...
                cost_inclusions: {
                  mobilisasi:
                    draft.operational.cost_inclusions.mobilisasi_demobilisasi,
                  penginapan:
                    draft.operational.cost_inclusions.penginapan_tukang,
                  pengiriman:
                    draft.operational.cost_inclusions.biaya_pengiriman,
                  langsir: draft.operational.cost_inclusions.biaya_langsir,
                  instalasi: draft.operational.cost_inclusions.instalasi,
                  ppn: draft.operational.cost_inclusions.ppn,
                },
                additional_costs: draft.operational.additional_costs.map(
                  (c) => ({
                    description: c.description,
                    amount: c.amount,
                    unit: c.unit,
            ...
            setCurrentStep(Math.max(0, draft.currentStep - 1))
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    try {
      ...
      const draftData = transformToDraftData(currentValues)
      const result = await saveDraft(draftData, currentStep + 1)
  ...
  }, [isPageLoading, isDirty, handleSaveDraft])
  ...
  const handleSubmit = async () => {
    ...
    const legalAgreement = form.getValues("review.legal_agreement")
    if (!legalAgreement) {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                {isSaving ? (
                  ...
                  <span>
                    Terakhir disimpan:{" "}
                    {lastSavedRef.current.toLocaleTimeString("id-ID", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </span>
          ...
          <div className="p-10">
            <form>
              <div className="space-y-8">
                ...
                {currentStep === 2 && (
                  <OperationalForm form={form} onForceSave={handleSaveDraft} />
                )}
